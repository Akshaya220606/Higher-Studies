...
async function loadApplications() {
  try {
    ...
    window.applicationsData = result.data; // store globally
    renderTbl(result.data);
...
function goto(pg) {
  ...
  if (pg === 's-register') {
    setTimeout(() => {
      autofillStudentForm();
    }, 100); // small delay so UI loads first
  }
...
function saveReg(){
  ...
  roll: r,
  email: e,
  branch: b || draft.branch,   // 🔥 keep old value if empty
  year: y || draft.year        // 🔥 keep old value if empty
};goto('s-exam');
}
...
function drawDash(){
  const s = window.applicationsData || [];

  const gate = s.filter(x => x.admission_type === 'GATE').length;
  const cat  = s.filter(x => x.admission_type === 'CAT').length;
  const gre  = s.filter(x => x.admission_type === 'GRE').length;

  const el = document.getElementById('a-sg');
  if(el) el.innerHTML = `
    <div class="sc"><div class="sc-v">${s.length}</div><div class="sc-l">Total Students</div></div>
    <div class="sc"><div class="sc-v">${gate}</div><div class="sc-l">GATE</div></div>
    <div class="sc"><div class="sc-v">${cat}</div><div class="sc-l">CAT</div></div>
    <div class="sc"><div class="sc-v">${gre}</div><div class="sc-l">GRE</div></div>
  `;
  ...
  // 🔥 COUNTRY DATA
  const cd = cntBy(
    s.map(x => ({ country: x.country || "Unknown" })),
    'country'
  );

  // 🔥 BRANCH DATA (from users)
  const bd = cntBy(
    s.map(x => ({ branch: x.users?.branch || "Unknown" })),
    'branch'
  );

  CH['dc'] = new Chart(document.getElementById('ch-c'), {
    type:'doughnut',
    data:{labels:Object.keys(cd),datasets:[{data:Object.values(cd),backgroundColor:CL}]}
  });

  CH['db'] = new Chart(document.getElementById('ch-b'), {
  type: 'bar',
  data: {
  ...
  options: {
    plugins: {
      legend: {
        display: false // 🔥 removes that rectangle
      }
    }
  }
});
...
function drawRep(){
  ...
  const scores = s.map(x => parseFloat(x.score)).filter(n => !isNaN(n));
  const avg = scores.length ? (scores.reduce((a,b)=>a+b,0)/scores.length).toFixed(1) : '—';

  const cd = cntBy(s.map(x => ({ country: x.country || "Unknown" })), 'country');
  const yd = cntBy(s.map(x => ({ year: x.users?.year || "Unknown" })), 'year');
  const ed = cntBy(s.map(x => ({ exam: x.admission_type || "Unknown" })), 'exam');
  const bd = cntBy(s.map(x => ({ branch: x.users?.branch || "Unknown" })), 'branch');

  const el=document.getElementById('r-sg');
  if(el) el.innerHTML = `
    <div class="sc"><div class="sc-v">${avg}</div><div class="sc-l">Avg Score</div></div>
    <div class="sc"><div class="sc-v">${Object.keys(cd)[0] || '—'}</div><div class="sc-l">Top Country</div></div>
    <div class="sc"><div class="sc-v">${Object.keys(bd)[0] || '—'}</div><div class="sc-l">Top Branch</div></div>
    <div class="sc"><div class="sc-v">${Object.keys(yd)[0] || '—'}</div><div class="sc-l">Latest Year</div></div>
  `;

  ['rc','ry','re','rb'].forEach(killC);

  CH['rc'] = new Chart(document.getElementById('r-c'), {
    type:'doughnut',
    data:{labels:Object.keys(cd),datasets:[{data:Object.values(cd),backgroundColor:CL}]}
  });

  CH['ry'] = new Chart(document.getElementById('r-y'), {
  type: 'bar',
  data: {
    labels: Object.keys(yd),
    datasets: [{
      data: Object.values(yd),
      label: "", // 🔥 IMPORTANT (remove "undefined")
    }]
  },
  options: {
    responsive: true,
    plugins: {
      legend: {
        display: false // 🔥 THIS REMOVES THE BOX
      }
    }
  }
});

  CH['re'] = new Chart(document.getElementById('r-e'), {
  type: 'bar',
  data: {
    labels: Object.keys(ed),
    datasets: [{
      data: Object.values(ed),
      label: "" // optional (just to be safe)
    }]
  },
  options: {
    plugins: {
...
CH['rb'] = new Chart(document.getElementById('r-b'), {
  type: 'bar',
  data: {
    labels: Object.keys(bd),
    datasets: [{
  ...
  options: {
    plugins: {
      legend: {
        display: false // 🔥 removes rectangle
      }
    }
  }
});
